...
function QrPlate({ seed }: { seed: string }) {
  ...
  return (
    <div className="grid rounded-neo bg-white p-md shadow-carve" style={{ gridTemplateColumns: `repeat(${n}, 1fr)`, width: 132 }}>
      {cells.map((on, i) => (
        <span key={i} className={`aspect-square ${on ? 'bg-ink' : 'bg-transparent'}`} />
      ))}
    </div>
  )
}
