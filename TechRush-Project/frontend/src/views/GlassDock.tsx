          ...
          >
            {items.map((it) => (
              <motion.button
                ...
                transition={SPRING}
                aria-label={it.label}
                className={`group relative flex size-12 items-center justify-center rounded-neo-full bg-plate text-ink ${it.active ? 'shadow-carve' : 'shadow-extrude-sm'}`}
              >
                {it.icon}
                <span className="pointer-events-none absolute -top-10 whitespace-nowrap rounded-neo-full bg-ink px-md py-xs text-[10px] font-bold uppercase tracking-widest text-on-dark opacity-0 shadow-float transition-opacity group-hover:opacity-100">
                  {it.label}
                </span>
              </motion.button>
            ))}
